...
export default function AiAssistant() {
  ...
  const generateAdvice = async () => {
    ...
    try {
      ...
      const prompt = `You are EcoTrack's AI Assistant. The user has the following carbon footprint baseline:
      Transport: ${state.baseline.transport.carMilesPerWeek} car miles/week, ${state.baseline.transport.carType} car.
      Home: ${state.baseline.home.electricityKwhPerMonth} kWh electricity/month.
      Food: ${state.baseline.food.dietType} diet.
      Their total emissions are ${state.emissions.total} kg CO2e/year.
      
      Please provide 3 specific, actionable recommendations on how they can reduce their carbon footprint. Format the response cleanly without markdown headers.`;
